...
class Builder:
    ...
    def _clean_directories(self):
        ...
        if os.path.exists(self.build_out_base_dir) or os.path.exists(self.build_dist_dir):
            ...
            if os.path.exists(self.build_out_base_dir):
                Utils.run_command(
                    ["rm", "-r", self.build_out_base_dir],
                    True,
                    self.log_clean_workspace,
                )
    ...
    def _build_frontend(self):
        ...
        Utils.run_command(
            ["cp", "-r", self.build_dist_dir + "/.", self.build_out_dist_dir],
            True,
            self.log_build_frontend,
        )
    ...
    def _build_backend(self):
        ...
        if os.path.exists(self.plugin_backend_dir) and os.path.exists(self.plugin_backend_dockerfile_dir):
            if os.path.exists(self.plugin_backend_output_dir):
                Utils.run_command(
                    ["rm", "-r", self.plugin_backend_output_dir],
                    True,
                    self.log_backend_build_backend,
                )
            Utils.run_command(
                ["mkdir", "-p", self.plugin_backend_output_dir],
                False,
                self.log_backend_build_backend,
            )
            Utils.run_command(
                [
                    ...
                    "build",
                    "-f",
                    self.plugin_backend_dockerfile_dir,
                    "-t",
                    self.plugin_backend_image,
                    self.plugin_dir,
                ],
                True,
                self.log_backend_build_backend,
            ...
            Utils.run_command(
                [
                    ...
                    "--privileged",
                    "-v",
                    self.plugin_backend_dir + ":/backend",
                    "-v",
                    self.plugin_backend_output_dir + ":/backend/out",
                    "-v",
            ...
            Utils.run_command(
                ["cp", "-r", self.plugin_backend_output_dir + "/.", self.build_out_dir],
                True,
                self.log_backend_build_backend,
            )
    ...
    def _copy_additional_files(self):
        ...
        Utils.run_command(
            ["cp", "-r", self.plugin_defaults_dir + "/.", self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            [
                "cp",
                "-r",
                self.plugin_py_dir,
                os.path.join(self.build_out_dir, "py_modules"),
            ],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            ["cp", self.plugin_main_py_file, self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            ["cp", os.path.join(self.plugin_dir, "LICENSE"), self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            ["cp", os.path.join(self.plugin_dir, "README.md"), self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            ["cp", os.path.join(self.plugin_dir, "package.json"), self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
        Utils.run_command(
            ["cp", os.path.join(self.plugin_dir, "plugin.json"), self.build_out_dir],
            True,
            self.log_copy_additional_files,
        )
    ...
    def _copy_remote_binaries(self):
        with open(self.log_binary_copy, "a") as log_file:
            ...
            for binary in remote_binaries:
                ...
                Utils.run_command(
                    ["wget", "-q", "-O", dest_file_path, url],
                    True,
                    self.log_binary_copy,
                )
                ...
                if actual_checksum == expected_checksum:
                    ...
                    print(
                        "        Error: Checksums do not match for file at URL: "
                        + url
                        + ". Expected: "
                        + expected_checksum
                        + ", Got: "
    ...
    def _package_plugin(self):
        ...
        try:
            Utils.run_command(
                ["tar", "-czvf", self.plugin_name + ".tar.gz", self.plugin_name],
                True,
                self.log_package_plugin,
            )
            Utils.run_command(
                ["zip", "-r", self.plugin_name + ".zip", self.plugin_name],
                True,
                self.log_package_plugin,
            )
